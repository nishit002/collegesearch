...
if uploaded_file is not None:
    ...
    college_names = df_colleges['College Name'].unique().tolist()
    ...
    if st.button("Generate Content"):
        if selected_colleges:
            ...
            for college in selected_colleges:
                ...
                row = df_colleges[df_colleges['College Name'] == college].iloc[0]
                
                # Create a Word document for the selected college
                doc = Document()
                
                # Add college details based on the template from all sheets
                doc.add_heading(row['College Name'], 0)

                if not pd.isna(row['Establishment year']):
                    doc.add_paragraph(f"{row['College Name']} was established in {row['Establishment year']}.")

                if not pd.isna(row['College City']) and not pd.isna(row['College State']):
                    doc.add_paragraph(f"The college is located in {row['College City']}, {row['College State']}.")

                if not pd.isna(row['College USP']):
                    doc.add_paragraph(f"The college is known for: {row['College USP']}.")

                if not pd.isna(row['NIRF RANK']):
                ...
                if 'Placements' in data_sheets:
                    ...
                    placement_row = df_placements[df_placements['College Name'] == college]
                ...
                if 'Awards' in data_sheets:
                    ...
                    award_row = df_awards[df_awards['College Name'] == college]
                ...
                if 'Faculty' in data_sheets:
                    ...
                    faculty_rows = df_faculty[df_faculty['College Name'] == college]
